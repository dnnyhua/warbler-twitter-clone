...
def logout():
    ...
    # IMPLEMENT THIS
    flash(f"See you later,  {user.username}!", "info")
    do_logout()
...
def profile():
    ...
    if form.validate_on_submit():
        # need to authenticate to apply changes, if authenicate returns false nothing updates
...
def homepage():
    ...
    if g.user:
        user = g.user
        follower_id = [follower.id for follower in user.following] + [user.id]

        messages = (Message.query.filter(Message.user_id.in_(follower_id))
                    .order_by(Message.timestamp.desc())
                    .limit(100)
                    .all())
